...
class Curva:
    ...
    def __init__(s, x:float=[], dim:int=3):
        ...
        s.lista_de_puntos() #Longjitud sobre la curva
        s.longitud()
    ...
    def lista_de_puntos(s) -> str:
        ...
        def longitud(s) -> None:
            ...
            for i in range(0,s.n):
                ...
                if i == s.n-1:
                    ip1 = 0
                d:float = (s.x[ip1]-s.x[i+j*s.n])**2
                for j in range(1,s.dim):
                    d += (s.x[ip1+j*s.n]-s.x[i+j*s.n])**2
                if i == 0: #para el caso de los puntos equidistantes
                    s.dx = d**0.5
                t += d**0.5
                s.l.append(t)
            s.L = t
